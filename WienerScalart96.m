function output=WienerScalart96(signal,fs,IS,NoiseMargin)
...
% Created: MAR-05
% Edited by Rong Gong 07/11/2014
...
if (nargin<4)
    NoiseMargin = 3; %Initial Silence or Noise Only part in seconds
end
...
x=segment(signal,W,SP,wnd); % This function chops the signal into frames
X=fft(x);
YPhase=angle(X(1:fix(end/2)+1,:)); %Noisy Speech Phase
X=abs(X(1:fix(end/2)+1,:));%Specrogram
numberOfFrames=size(X,2);
FreqResol=size(X,1);

N=mean(X(:,1:NIS)')'; %initial Noise Power Spectrum mean
P=mean((X(:,1:NIS)').^2)';%initial Noise Power Spectrum variance
beta=.98; %used in smoothing xi (For Deciesion Directed method for estimation of A Priori SNR)
NoiseCounter=0;
...
S=zeros(size(X)); % Initialize X (memory allocation)
...
for i=1:numberOfFrames
    ...
    if i<=NIS % If initial silence ignore VAD
        ...
        [NoiseFlag, SpeechFlag, NoiseCounter, Dist]=vad(X(:,i),N,NoiseCounter,NoiseMargin); %Magnitude Spectrum Distance VAD
    end
    
    if SpeechFlag==0 % If not Speech Update Noise Parameters
        N=(NoiseLength*N+X(:,i))/(NoiseLength+1); %Update and smooth noise mean
        P=(NoiseLength*P+(X(:,i).^2))./(1+NoiseLength); %Update and smooth noise variance
    end
    %%%%%%%%%%%%%%%%%%%VAD and Noise Estimation END
    
    gammaNew=(X(:,i).^2)./P; %A postiriori SNR
    S_tminus1 = G.^2.*Gamma;
    SNRprio=beta*S_tminus1+(1-beta).*max(gammaNew-1,0); %Decision Directed Method for A Priori SNR
    Gamma=gammaNew;
    
    G=(SNRprio./(SNRprio+1));
    
    S(:,i)=G.*X(:,i); %Obtain the new Cleaned value
...
figure
imagesc(20*log10(S(:,(NIS+1):end)));
caxis([-20 20])
colorbar
set(gca,'YDir','normal')
xlabel('frame')
ylabel('frequency Hz')
title('output of a priori SNR estimation Wiener filter (dB)')


output=OverlapAdd2(S,YPhase,W,SP*W); %Overlap-add Synthesis of speech
